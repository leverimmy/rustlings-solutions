    ...
    pub fn transformer(input: Vec::<(String, Command)>) -> Vec<String> {
        ...
        for (string, command) in input.iter() {
            ...
            match command {
                ...
                Command::Append(size) => {
                    // Warning @ here!
                    transformed_string.push_str(&"bar".repeat(*size));
                    // It is `*size` instead of `size`, because `input.iter()` returns a tuple of references.
                    // All the things below are references, so does `size`.
                },
            };
